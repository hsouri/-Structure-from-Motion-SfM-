...
class SfMLearner(object):
    ...
    def adjust_lr(self,curr_step,learning_rate,step_inc):
    ...
    def SSIM(self, x, y):
        m_x = slim.avg_pool2d(x, 3, 1, 'SAME')
        m_y = slim.avg_pool2d(y, 3, 1, 'SAME')
        sigma_x = slim.avg_pool2d(x ** 2, 3, 1, 'SAME') - m_x ** 2
        sigma_y = slim.avg_pool2d(y ** 2, 3, 1, 'SAME') - m_y ** 2
        sigma_xy = slim.avg_pool2d(x * y, 3, 1, 'SAME') - m_x * m_y
        p1 = 0.01 ** 2; p2 = 0.03 ** 2
        SSIM = (2 * m_x * m_y + p1) * (2 * sigma_xy + p2) / (m_x ** 2 + m_y ** 2 + p1) * (sigma_x + sigma_y + p2)
        reduced = tf.reduce_mean(tf.clip_by_value((1 - SSIM) / 2, 0, 1))
        return reduced
    ...
    def train(self, opt):
        ...
        with sv.managed_session(config=config) as sess:
            ...
            for step in range(1, opt.max_steps):
                ...
                opt.learning_rate = self.adjust_lr(step,opt.learning_rate,opt.step_lr)
