...
class DataLoader(object):
    ...
    def data_augmentation(self, im, intrinsics, out_h, out_w):
        ...
        def random_coloring(im):
            ...
            im  = tf.image.convert_image_dtype(im, tf.float32)  ** tf.random_uniform([], 0.8, 1.2)
            im  =  im * tf.random_uniform([], 0.5, 2.0)
            random_colors = tf.random_uniform([in_c], 0.8, 1.2)
            color_image = tf.stack([tf.ones([batch_size, in_h, in_w]) * random_colors[i] for i in range(in_c)], axis=3)
            im  *= color_image
